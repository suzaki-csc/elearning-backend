...
from src.schemas.content import (
    ContentCreate, ContentUpdate, CategoryCreate, CategoryUpdate
)
...
class ContentService:
    ...
    async def update_category(
        ...
        if (category_data.parent_id and
    ...
    async def update_content(
        ...
        if (content_data.category_id and 
            content_data.category_id != content.category_id):
            category = await self.get_category_by_id(content_data.category_id)
            if not category or not category.is_active:
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
